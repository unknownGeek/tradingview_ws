...
def print_candles():
  ...
  max_price = max(highs)
  chart_height = 10
  price_step = (max_price - min_price) / (chart_height - 1) if max_price > min_price else 1
  chart_rows = []
  for row in range(chart_height, 0, -1):
    price_level = min_price + (row - 1) * price_step
    line = ""
    for c in candles:
      ...
      if l <= price_level <= h:
        if min(o, cl) <= price_level <= max(o, cl):
          line += color + BODY + RESET
        else:
          line += color + WICK + RESET
      else:
        line += SPACE
      line += SPACE
    chart_rows.append(f"{price_level:8.2f} | {line}")
  print(f"{BOLD}\n--- Live Candlestick Chart ---{RESET}")
  for r in chart_rows:
    print(r)
  print("         +" + "--" * len(candles))
  print("          " + " ".join([datetime.fromtimestamp(c['timestamp']).strftime('%H:%M') for c in candles]))
  print("---------------\n")
  # Optionally print text summary
  for c in candles:
